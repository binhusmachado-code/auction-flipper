      ...
      <div className="flex h-[min(760px,96vh)] w-full max-w-3xl flex-col overflow-hidden rounded-lg border border-zinc-800 bg-zinc-900 shadow-2xl" onClick={(event) => event.stopPropagation()}>
        ...
        <main className="min-h-0 flex-1 overflow-y-auto p-4 sm:p-7">
          ...
          {step === 1 && <div className="space-y-4">
            ...
            <p className="rounded-lg border border-zinc-800 bg-zinc-950/50 p-3 text-xs leading-relaxed text-zinc-500">Live inventory depends on the official sources currently connected. The directory covers every state and D.C., but a directory entry is not the same as a live property. Confirm the sale type on the card and official file.</p>
          </div>}
          ...
          {step === 2 && <div className="space-y-4">
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              ...
              <label><span className="mb-1.5 block text-xs font-semibold text-zinc-400">County or state interest</span><input className={selectClass} value={preferences.county} onChange={(event) => setPreferences({ ...preferences, county: event.target.value })} placeholder="Example: Brevard, FL or learning only" /></label>
          ...
          {step === 3 && <div className="space-y-4">
            ...
            <div className="grid gap-2 sm:grid-cols-2">
              {['Parcel and legal description agree', 'Legal and physical access', 'Zoning, buildability, flood and wetlands', 'Occupancy and current dated photos', 'Title search and surviving claims', 'Code, utilities, assessments and association', 'Current auction status and payment deadline', 'Exit plan and title insurability', 'Official unsold or over-the-counter list', 'Sale-day withdrawal and redemption check'].map((item) => <div key={item} className="flex gap-2 rounded-lg border border-zinc-800 bg-zinc-950/50 p-3 text-sm text-zinc-300"><ShieldCheck className="mt-0.5 h-4 w-4 flex-none text-amber-400" />{item}</div>)}
            </div>
          </div>}
